...
for(k in 1:kfold){
  ...
  res_temp = optim(rep(1,p), KL_lm_embarc)
  ...
  hatD1 = as.matrix(VarCorr(fit_drg_est)$subj)[1:3, 1:3] 
  hatbeta2 = as.matrix(fixef(fit_pbo_est))[1:3] 
  hatgamma2 = as.matrix(fixef(fit_pbo_est))[4:6] 
  hatD2 = as.matrix(VarCorr(fit_pbo_est)$subj)[1:3, 1:3] 
  ...
  for(ii in unique(cv_test$subj)){
    ...
    truedrg = c(truedrg, temp$cs_drg[1])
    truepbo = c(truepbo, temp$cs_pbo[1])
  ...
  restable = data.frame(subj = id, truegroup = truegroup,
                        ...
                        estdrg = estdrg, estpbo = estpbo,
                        truedrg = truedrg, truepbo = truepbo)
