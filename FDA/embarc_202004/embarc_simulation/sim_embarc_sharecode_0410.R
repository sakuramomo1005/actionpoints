...
# setwd('~/Desktop/NYU/Research/FDA/10. embarc changescore/simulated')
...
if(settings == 3){
  beta_drg = c(18.6, -2.3, 1.7)
  beta_pbo = c(18.6, -1.9, 1.4)
...
for(iters in 1:100){
  ...
  result_kl = list(Restable = Restable, 
                cvpboscore = cvpboscore,
                cvdrgscore = cvdrgscore,
                covar_names = covar_names)
  
  Result_kl[[iters]] = result_kl
  # linear method
  cvdrgscore = cvpboscore = c()
  Restable = list()
  ...
  result_linear = list(Restable = Restable, 
                cvpboscore = cvpboscore,
                cvdrgscore = cvdrgscore,
                covar_names = covar_names)
  
  Result_linear[[iters]] = result_linear
  # Result[[iters]] = list(result_kl = result_kl, result_linear = result_linear)
  
  Result = list( Result_kl =  Result_kl, Result_linear = Result_linear)
  names = paste('sim_embarc_set_',settings,'_s_',sigma0,'_p_', p, '_angles_',angles,'.RData', sep='')
