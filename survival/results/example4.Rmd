---
title: "Example 4- Exponential and Weibull"
output: pdf_document
fontsize: 12pt
---
...
Consider the joint distribution who has: 
$$
...
And when $x < y$, the pdf is 
$$f_{T,C}(x,y) = \frac{\partial^2}{\partial x \partial y} S(x,y) = \frac{\partial^2}{\partial x \partial y} e^{- \theta x} e^{- (\theta y)^k} = k\theta^{k+1} y^{k-1} \exp(-\theta x- (\theta y)^k)$$
Then 
$$\begin{aligned}
\psi(x) = & \int_x^\infty f_{T,C}(x,y) dy \\
= & \int_x^\infty  k\theta^{k+1} y^{k-1} \exp(-\theta x- (\theta y)^k) dy \\
=& \theta \exp(-\theta x- (\theta x)^k)
\end{aligned}$$
The $\rho(x)$ is:
...
When $x < y$, (death time $<$ censor time), the death time is generated from an exponentaion with parameter $\theta$, the censor time is independently generated from a Weibull distribution. The pairs that $x < y$ are selected. 



When $x > y$, firstly generate death time  an exponentaion with parameter $\theta$. 
Given a known death time, generate censor time from the conditional distribution: 
$$\begin{aligned}
F_{C|T}(y | x) = & \int_0^y f_{C|T}(v|x) dv \\
= & \int_0^y \frac{\partial }{\partial y} \frac{\partial F_{T,C}(x,y)}{\partial x f_T(x)} |_{y = v} dv \\
= & \frac{\partial F_{T,C}(x,y)}{\partial x f_T(x)} |_{0}^y \\
= & (2 \theta (\theta y)^k(y-x) - 1) \exp(- (\theta y)^k \big( ( \theta x-  \theta y)^2 + 1\big)) |_{0}^y \\
= & 1 - (2 \theta (\theta y)^k(x -y) + 1) \exp(- (\theta y)^k \big( ( \theta x-  \theta y)^2 + 1\big))
\end{aligned}$$

The censoring percentage 
$$\begin{aligned}P(T < C) = & \int_0^\infty \int_0^y f_{T.C}(x, y)dx dy = \int_0^\infty \int_0^y k \theta^{k+1} y^{k-1}e^{-\theta x - (\theta y)^k} dx dy \\
= & \int_0^\infty  k \theta^{k} y^{k-1} (1 - e^{-\theta y}) e^{-(\theta y)^k} dy
\end{aligned}$$

To make things easy, we can set $k = 2$, then 
$$\begin{aligned}
P(T < C) = & \int_0^\infty  2 \theta^{2} y (1 - e^{-\theta y}) e^{-(\theta y)^2} dy \\
= & \int_0^\infty 2 \theta^{2} y e^{-(\theta y)^2} - 2 \theta^{2} ye^{-\theta y-(\theta y)^2}dy \\ 
= & -e^{-(\theta y)^2} |_0^\infty - \int_0^\infty 2 \theta^{2} ye^{-\theta y-(\theta y)^2}dy \\
\approx & 0.546
\end{aligned}$$
...
for(i in 1:2){
  names = paste('theta_3_', theta_list[i], '.RData', sep ='')
  load(names)
  
  
  # theta estimate 
  the_est1 = c(the_est1, mean(abs( result$theta_est1 )))
  the_est2 = c(the_est2, mean(abs( result$theta_est2 )))
  #hist(abs(-result$theta_est1 -  log(theta_list[i])), breaks = 50)
  
  range(result$theta_est1)
  sd(result$theta_est1)
...
for(functions in 1){
  Fc_t = function(y,x){
    res = 1 - (2*theta*(theta*y)^k*(x-y) + 1)*exp(-theta^k*y^k*((theta*x-theta*y)^2 + 1))
    return(res)
  }
  inverse_fct = function(a,y){
    ...
    temp = seq(0, 1, 0.1)[which(abs(res1 - a) == min(abs(res1 - a)))[1]]
...
for(datageneration in 1){
  ...
  for(i in 1:length(x)){
    ...
    # xx = runif(1,0,bars)
    xx = runif(1,0,1)
    c1 = c(c1, inverse_fct(xx, x[i]))
  }
  # get the pairs that x > y. 
  data1 = data.frame(death = x, censor = c1)
  ...
  data1 = data1[sample(1:dim(data1)[1])[1:227],]
...
lines(seq(0,7,0.1), theta* exp(- theta * seq(0,7,0.1)), col = 2)
```
...
lines(seq(0,7,0.1),weibull(seq(0,7,0.1),shape, scale))
```
...
```{R echo = FALSE}
the_est1 = matrix(the_est1, 1,2)
colnames(the_est1) = c(1,2)
rownames(the_est1) = 'Estimated intercept'
kable(round(the_est1,4), booktabs=TRUE, "latex", align="c", longtable = T, escape = F)
the_est2 = matrix(the_est2, 1,2)
colnames(the_est2) = c(1,2)
rownames(the_est2) = 'Estimated slope'
kable(round(the_est2,4), booktabs=TRUE, "latex", align="c", longtable = T, escape = F)
```
...
```{R echo = FALSE}
m_est = matrix(m_est, 1,2)
colnames(m_est) = c(1,2)
...
colnames(new_m) = c(1,2,1,2)
new_m = round(new_m, 4)
kable(new_m, "latex", align="c", booktabs=TRUE, escape = F,  
      linesep = NULL, longtable = T) %>%
  add_header_above(header = c(" " = 1,
                              " With true m()" = 2,
                              " Logistic estimated m()" = 2))
```
